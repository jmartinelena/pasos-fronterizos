            ...
            cursor.execute("""
                            ...
                            BEGIN
	                            ...
	                            SELECT fecha_id INTO fechaId 
                                    FROM fecha WHERE fecha_scaneo = fechaScaneo AND ultima_actualizacion = ultimaActualizacion;
	                            SELECT locacion_id INTO locacionId FROM locacion WHERE nombre_paso = nombrePaso;

	                            INSERT INTO
	                            	tipo(fecha_id, locacion_id, tipo, temperatura, tiempo, 
                                    viento, visibilidad, altura_del_rio, alerta_del_rio, evacuacion_del_rio)
	                            VALUES
	                            	(fechaId, locacionId, tipoPaso, temperaturaPaso, tiempoPaso, 
                                    vientoPaso, visibilidadPaso, alturaRio, alertaRio, evacuacionRio)
	                            ON DUPLICATE KEY UPDATE
	                            	tipo = tipoPaso, temperatura = temperaturaPaso, tiempo = tiempoPaso, 
